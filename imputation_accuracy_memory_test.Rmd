...
## Fast, no standardisation

```{r fast_nostandard,eval=TASKID==0|TASKID==2}
for (n in ns) {
  for (m in ms) {
    launch(n, m, fast=TRUE, standardized=FALSE)
  }
}
```

## Non-fast, with standardisation

```{r nofast_standard,eval=TASKID==0|TASKID==3}
for (n in ns) {
  for (m in ms) {
    launch(n, m, fast=FALSE, standardized=TRUE)
  }
}
```

## Fast, with standardisation

```{r fast_standard,eval=TASKID==0|TASKID==4}
for (n in ns) {
  for (m in ms) {
    launch(n, m, fast=TRUE, standardized=TRUE)
  }
}
```
...
```{r cache=FALSE}
results <- list.files(.results('/'), pattern = 'cpumemlog.*.txt', full.names = TRUE) %>%
  lapply(function(s) read.table(s, header=TRUE, as.is=TRUE, fill=TRUE) %>% mutate(fn=basename(s))) %>%
  do.call(what=bind_rows) %>%
  filter(!is.na(COMMAND), ETIME != '') %>%
  separate(fn, into=c('nah','n','m','fast','standardized'), sep='[_.]', extra='drop') %>%
  separate(ETIME, into=c('hr','min','sec'), sep=':') %>% mutate(ETIME=3600*as.integer(hr)+60*as.integer(min)+as.integer(sec)) %>%
  mutate(n=as.integer(n), m=as.integer(m))

results %>% group_by(n, m, fast, standardized) %>% summarise_each(funs(max), RSS, VSZ, ETIME) %>% with(table(n,m,fast,standardized))
  
results %>% ggplot(aes(x=ETIME, y=RSS, group=PID, linetype=standardized, colour=fast)) + geom_line()

results %>% filter(n %in% c(500,2000,5000)) %>% group_by(n, m, fast, standardized) %>% summarise_each(funs(max), RSS, VSZ, ETIME) %>%
  gather('stat','val',RSS, VSZ,ETIME) %>%
  ggplot(aes(x=m, y=val, colour=fast, linetype=standardized, group=interaction(fast, standardized))) + geom_line() + 
  facet_grid(stat~n, scales='free_y',labeller=labeller(n=label_both)) +
  scale_x_log10()

results %>% filter(m %in% c(5000, 1000000)) %>% group_by(n, m, fast, standardized) %>% summarise_each(funs(max), RSS, VSZ, ETIME) %>%
  gather('stat','val',RSS, VSZ,ETIME) %>%
  ggplot(aes(x=n, y=val, colour=fast, linetype=standardized, group=interaction(fast, standardized))) + geom_line() + 
  facet_grid(stat~m, scales='free_y',labeller=labeller(m=label_both)) +
  scale_x_log10()
