...
```{r fast_nostandard,eval=TASKID==0|TASKID==4}
for (n in ns) {
  for (m in ms) {
...
results <- list.files(.results('/'), pattern = 'cpumemlog.*.txt', full.names = TRUE) %>%
  lapply(function(s) read.table(s, header=TRUE, as.is=TRUE) %>% mutate(fn=basename(s))) %>%
...
results %>% ggplot(aes(x=ETIME, y=RSS, group=PID)) + geom_line()
results %>% group_by(n, m, fast, standardized) %>% summarise_each(funs(max), RSS, VSZ, ETIME) %>%
  ungroup %>%
  mutate(n=as.integer(n), m=as.integer(m)) %>%
  gather('stat','val',RSS, VSZ,ETIME) %>%
  ggplot(aes(x=n*m, y=val)) + geom_line() + facet_grid(stat~., scales='free_y')
```
