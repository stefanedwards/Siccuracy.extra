...
tmp <- tempdir()
olddir <- setwd(tmp)
file.copy(file.path(src, files), '.', overwrite=TRUE)
...
tmp <- tempdir()
olddir <- setwd(tmp)
file.copy(file.path(src, files), file.path(tmp, knitr:::sub_ext(files, 'f90')), overwrite=TRUE)
files <- list.files(tmp, pattern='*.f90')
out <- file.path(TMPDIR, 'ifort.so')
cat('FC=ifort\n', file='localvars')
system2('R', args=c('CMD SHLIB','-o', out, '--dry-run', files), env=c('R_MAKEVARS_USER=localvar', 'PKG_FCFLAGS=-O3'))
```
...
if (nrow(x) > 0) {
  x %>% separate(ETIME, into=c('hr','min','sec'), sep=':') %>% mutate(ETIME=3600*as.integer(hr)+60*as.integer(min)+as.integer(sec)) %>%
    mutate(type=type, rowid=rowid) %>%
    dbSendPreparedQuery(dbconn, 'INSERT INTO results (jobid, type, ETIME, RSS, VSZ) VALUES (:rowid, :type, :ETIME, :RSS, :VSZ);')
}
...
while (TRUE) {
  ...
  dbSendPreparedQuery(dbconn, 'UPDATE jobs SET status=1 WHERE jobid=:jobid;')
  dbCommit(dbconn)
  ...
  st <- system2('./launcher.sh', args=c(n,m,fast,standardized,fn))
  if (st == 0) dbSendPreparedQuery(dbconn, 'UPDATE jobs SET status=2 WHERE jobid=:jobid;')
}
