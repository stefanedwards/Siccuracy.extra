...
```{r R_R,eval=.do.computations,cache=FALSE,engine='cat',engine.opts=list(file='wrapup.R',lang='R')}
load('run_options.Rdata')
...
while (TRUE) {
  ...
  for (s in c('gfort.so','gfortO3.so','ifort.so')) 
    st <- st + system2('./launcher.sh', args('test.R',s), stdout=FALSE)
  st <- st + system2('./launcher.sh', args('R.R', 'R'), stdout=FALSE)
...
```{r include=FALSE}
dbconn <- dbConnect(RSQLite::SQLite(), .results('results.sqlite'))
```
...
dbGetQuery(dbconn, 'SELECT n, m, standardized, adaptive, type, ETIME FROM view_results;') %>%
  group_by(n, m, standardized, adaptive, type) %>% top_n(1, ETIME) %>% 
  ggplot(aes(x=as.factor(n), fill=type)) + geom_bar(position='dodge') + 
  facet_grid(adaptive ~ standardized, labeller=label_both) +
  coord_flip() + labs(x='Animals (`n`)', colour='Type')
```
...
dbGetQuery(dbconn, 'SELECT n, m, standardized, adaptive, replicate, type, RSS FROM view_results;') %>%
  group_by(n, m, standardized, adaptive, replicate, type) %>% top_n(1, RSS) %>%
  ggplot(aes(x=n, y=RSS, colour=type)) + geom_point(pch=1) +
  facet_grid(adaptive ~ standardized, scales='free_y', labeller=label_both)
```
...
dbGetQuery(dbconn, 'SELECT n, m, standardized, adaptive, replicate, type FROM view_results;') %>%
  distinct(n, m, standardized, adaptive, replicate, type) %>% 
  group_by(n, m) %>% summarise(types=length(unique(type)), no=n()) %>% ungroup %>%
  filter(types == 4) %>% mutate(max(no)) %>% arrange(desc(n), desc(m))
view <- data.frame(n=20000, m=30000)
```
...
dbGetPreparedQuery(dbconn, 'SELECT * FROM view_results WHERE n=:n;', view) %>% 
  ...
  gather(stat, val, ETIME, RSS, VSZ) %>% #filter(adaptive=='TRUE', standardized=='TRUE', stat=='VSZ', type!='gfortO3.so') %>% #filter(type=='gfort.so', adaptive=='TRUE', standardized=='TRUE') %>% View
...
```{r}
p <- dbGetPreparedQuery(dbconn, 'SELECT * FROM view_results WHERE m=:m AND standardized="TRUE";', view) %>% 
  group_by(n, m, standardized, adaptive, replicate, type) %>% top_n(1, ETIME)  %>%# gets last observation; latter two do not decrease.
  mutate(ETIME=ETIME/60) %>%
  ggplot(aes(x=n, y=ETIME, colour=type)) + geom_point(pch=1) +
  stat_summary(fun.y=mean, geom='line') +
  facet_wrap(~ adaptive, ncol=2, scales='free_y', labeller=labeller(adaptive=c('TRUE'="2nd implementation", 'FALSE'="'Fast' implementation"))) +
  labs(colour='Compiler', x=sprintf('# Individuals (%s SNPs)', prettyNum(view$m, big.mark = ',')), y='Running time (min)') +
  theme(panel.border=element_blank(), axis.line.x=element_line()) +
  coord_cartesian(ylim=c(0,7)) +
  scale_x_continuous(labels=function(i) prettyNum(i, big.mark=','))
